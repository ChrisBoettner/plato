...
def sunSkyCoordinates(julianDate):
    ...
    eclipticLongitude = (
        meanLongitude
        + 0.03341607 * np.sin(meanAnomaly)
        + 0.00034894 * np.sin(2 * meanAnomaly)
        - 0.0001134
        - 0.0000203 * np.sin(Omega)
    )
    eclipticObliquity = (
        0.4090928 - 6.2140e-9 * elapsedJulianDays + 0.00003963 * np.cos(Omega)
    )

    # Compute the RA, DEC of the Sun. Ensure that the RA is positive.

    raSun = np.arctan2(
        np.cos(eclipticObliquity) * np.sin(eclipticLongitude), np.cos(eclipticLongitude)
    )
...
def ecliptic2equatorial(lam, beta):
    ...
    sindelta = np.sin(beta) * np.cos(obliquity) + np.cos(beta) * np.sin(
        obliquity
    ) * np.sin(lam)
    ...
    sinalpha = (
        -np.sin(beta) * np.sin(obliquity)
        + np.cos(beta) * np.cos(obliquity) * np.sin(lam)
    ) / cosdelta
...
def equatorial2ecliptic(alpha, delta):
    ...
    sinbeta = np.sin(delta) * np.cos(obliquity) - np.cos(delta) * np.sin(
        obliquity
    ) * np.sin(alpha)
    ...
    sinlambda = (
        np.sin(delta) * np.sin(obliquity)
        + np.cos(delta) * np.cos(obliquity) * np.sin(alpha)
    ) / cosbeta
    ...
    if lam < 0.0:
        lam += 2.0 * 3.141592653589793

    return lam, beta


def sunSkyCoordinatesAwayfromPlatformPointing(
    raPlatform, decPlatform, solarPanelOrientation=0.0
):
...
def platformAnglesFromQuaternion(q_EQ2PLM):
    ...
    v_PLM = [0.0, 0.0, 0.0, 1.0]
    # Pointing axis of the PLM is the z-axis
    ...
    q_A2B = [
        np.cos(np.pi / 4 - decPlatform / 2.0),
        0.0,
        np.sin(np.pi / 4 - decPlatform / 2.0),
        0.0,
    ]
...
def platformQuaternionFromAngles(raPlatform, decPlatform, solarPanelOrientation):
    ...
    q_B2PLM = [
        np.cos(solarPanelOrientation / 2.0),
        0.0,
        0.0,
        np.sin(solarPanelOrientation / 2.0),
    ]
    q_EQ2PLM = qmul(q_EQ2A, qmul(q_A2B, q_B2PLM))
    return q_EQ2PLM


def skyToPlatformCoordinates(
    raStar, decStar, raPlatform, decPlatform, solarPanelOrientation
):
    ...
    rotEQ2A = np.array(
        [
            [np.cos(raPlatform), np.sin(raPlatform), 0.0],
    ...
    rotA2B = np.array(
        [
            [np.sin(decPlatform), 0.0, -np.cos(decPlatform)],
            [0.0, 1.0, 0.0],
    ...
    rotB2PLM = np.array(
        [
            [np.cos(solarPanelOrientation), np.sin(solarPanelOrientation), 0.0],
    ...
    starEQ = np.array(
        [
            np.cos(decStar) * np.cos(raStar),
            np.cos(decStar) * np.sin(raStar),
            np.sin(decStar),
        ]
    )
...
def platformToSkyCoordinates(
    xPLM, yPLM, zPLM, raPlatform, decPlatform, solarPanelOrientation
):
...
def skyToFocalPlaneCoordinates(
    raStar,
    decStar,
    raPlatform,
    decPlatform,
    ...
    rotAzimuth = np.array(
        [
            [np.cos(azimuthAngle), np.sin(azimuthAngle), 0],
    ...
    rotTilt = np.array(
        [
            [np.cos(tiltAngle), 0, -np.sin(tiltAngle)],
            [0, 1, 0],
    ...
    rotCAM2FP = np.array(
        [
            [np.cos(focalPlaneAngle), np.sin(focalPlaneAngle), 0],
...
def focalPlaneToSkyCoordinates(
    xFP,
    yFP,
    raPlatform,
    decPlatform,
...
def pixelCoordinates2FocalPlaneAngles(xCCD, yCCD, ccdCode, pixelSize, focalLength):
    ...
    xFPmm, yFPmm = pixelToFocalPlaneCoordinates(
        xCCD, yCCD, pixelSize, ccdZeroPointX, ccdZeroPointY, ccdAngle
    )
    angleFromOpticalAxis = gnomonicRadialDistanceFromOpticalAxis(
        xFPmm, yFPmm, focalLength
    )
...
def focalPlaneAngles2pixelCoordinates(
    angleFromOpticalAxis, azimuthFromXAxis, ccdCode, pixelSize, focalLength
):
    ...
    xFPmm, yFPmm = focalPlaneCoordinatesFromGnomonicRadialDistance(
        angleFromOpticalAxis, focalLength, inPlaneRotation=azimuthFromXAxis
    )
    xCCD, yCCD = focalPlaneToPixelCoordinates(
        xFPmm, yFPmm, pixelSize, ccdZeroPointX, ccdZeroPointY, ccdAngle
    )
...
def telescopeToUndistortedFocalPlaneCoordinates(
    xCAM, yCAM, zCAM, focalLength, focalPlaneAngle
):
...
def generateDistortionTable(psfFile, focalLength):
    ...
    return {
        "Undistorted": {"x": xUndistortedCoordinates, "y": yUndistortedCoordinates},
        "Distorted": {"x": xDistortedCoordinates, "y": yDistortedCoordinates},
    }


def undistortedToDistortedFocalPlaneCoordinates(
    xFPmm, yFPmm, distortionCoefficients, focalLength
):
    ...
    tangentialDistortion = (
        rFP**2
        * (
            distortionCoefficients[5] * np.cos(angle)
            + distortionCoefficients[6] * np.sin(angle)
        )
        * focalLength
    )
    ...
    xFPdist = (
        xFPmm
        + np.cos(angle) * (radialDistortion + tangentialDistortion)
        + distortionCoefficients[3] * rFP**2 * focalLength
    )
    yFPdist = (
        yFPmm
        + np.sin(angle) * (radialDistortion + tangentialDistortion)
...
def mappedUndistortedToDistortedFocalPlaneCoordinates(
    xFPmm, yFPmm, pathToPsfFile, focalLength=None
):
    ...
    if not "Coordinates map" in psfFile.keys():
        if False:
            print(
                "Error: No transformation map given in psf file, mapped distortion is not possible."
            )
            return
        else:
    ...
    aDistanceFromPoint = np.array(
        [x**2 + y**2 for x, y in zip(distanceFromPointx, distanceFromPointy)]
    )
    ...
    constants = [
        abs((closestX[oPointIdx[i]] - xFPmm) * (closestY[oPointIdx[i]] - yFPmm))
...
def distortedToUndistortedFocalPlaneCoordinates(
    xFPdist, yFPdist, inverseDistortionCoefficients, focalLength
):
    ...
    inverseCoefficientsRadial = [
        0,
        0,
        0,
        inverseDistortionCoefficients[0],
        0,
        inverseDistortionCoefficients[1],
        0,
        inverseDistortionCoefficients[2],
    ]
    inverseDistortionPolynomialRadial = np.polynomial.Polynomial(
        inverseCoefficientsRadial
    )
    ...
    tangentialDistortion = (
        rFP**2
        * (
            inverseDistortionCoefficients[5] * np.cos(angle)
            + inverseDistortionCoefficients[6] * np.sin(angle)
        )
        * focalLength
    )
...
def mappedDistortedToUndistortedFocalPlaneCoordinates(
    xFPdist, yFPdist, pathToPsfFile, focalLength=None
):
    """
    ...
    while (delta > 0.001) and (i < 160):
        xDist, yDist = mappedUndistortedToDistortedFocalPlaneCoordinates(
            x0, y0, pathToPsfFile, focalLength
        )
        length = 3 * length / 5
...
def pixelToFocalPlaneCoordinates(
    xCCDpixel, yCCDpixel, pixelSize, ccdZeroPointX, ccdZeroPointY, CCDangle
):
    """From pixel to focal plane coordinates.
...
def focalPlaneToPixelCoordinates(
    xFP, yFP, pixelSize, ccdZeroPointX, ccdZeroPointY, CCDangle
):
...
def focalPlaneCoordinatesFromGnomonicRadialDistance(
    angularDistance, focalLength, inPlaneRotation=0.0
):
...
def computeCCDcornersInFocalPlane(ccdCode, pixelSize):
    ...
    cornersXmm, cornersYmm = pixelToFocalPlaneCoordinates(
        cornersXpix, cornersYpix, pixelSize, zeroPointXmm, zeroPointYmm, ccdAngle
    )
...
def getCCDandPixelCoordinates(
    ...
    if includeFieldDistortion or includeFieldDistortion == "yes":
        if mappedDistortion and pathToPsfFile is None:
            print(
                "Error: If mapped field distortion should be taken into account, "
                + "a path to the psf file should be given"
            )
            return
        elif (not mappedDistortion) and distortionCoefficients is None:
            print(
                "Error: If analytic field distortion should be taken into account, "
    ...
    xFPmm, yFPmm = skyToFocalPlaneCoordinates(
        raStar,
        decStar,
        raPlatform,
        decPlatform,
    ...
    if (includeFieldDistortion == True) or (includeFieldDistortion == "yes"):
        if mappedDistortion:
            xFPmm, yFPmm = mappedUndistortedToDistortedFocalPlaneCoordinates(
                xFPmm, yFPmm, pathToPsfFile, focalLength
            )
        else:
            xFPmm, yFPmm = undistortedToDistortedFocalPlaneCoordinates(
                xFPmm, yFPmm, distortionCoefficients, focalLength
            )
    ...
    for ccdCode in ccdCodes:
        ...
        xCCDpix, yCCDpix = focalPlaneToPixelCoordinates(
            xFPmm, yFPmm, pixelSize, zeroPointXmm, zeroPointYmm, ccdAngle
        )
        ...
        Ncols = CCD[ccdCode]["Ncols"]
        firstRow = CCD[ccdCode]["firstRow"]
        if (xCCDpix < 0) or (yCCDpix < firstRow):
            continue
        if (xCCDpix >= Ncols) or (yCCDpix >= Nrows):
            continue
            ...
            if (includeFieldDistortion == True) or (includeFieldDistortion == "yes"):
                if mappedDistortion:
                    xFPmms[cam], yFPmms[cam] = (
                        mappedUndistortedToDistortedFocalPlaneCoordinates(
                            xFPmms[cam], yFPmms[cam], pathToPsfFile, focalLength
                        )
                    )
                else:
                    xFPmms[cam], yFPmms[cam] = (
...
def skyToFocalPlaneCoordinates_par(
    targetCoords,
    raPlatform,
    decPlatform,
    solarPanelOrientation,
...
def platformToTelescopePointingCoordinates(
    raPlatform, decPlatform, solarPanelOrientation, azimuthAngle, tiltAngle
):
    ...
    if isinstance(raOpticalAxis, np.ndarray):
        raOpticalAxis[raOpticalAxis < 0.0] += 2.0 * np.pi
    else:
        if raOpticalAxis < 0.0:
            raOpticalAxis += 2.0 * np.pi
...
def getCameraGroupCoordinates(raPlatform, decPlatform, solarPanelOrientation=0):
    ...
    for i in range(4):
        ra[i], dec[i] = platformToTelescopePointingCoordinates(
            raPlatform,
            decPlatform,
            solarPanelOrientation,
            azimuthAngles[i],
...
def calculateSubfieldAroundCoordinates(
    subfieldSizeX,
    subfieldSizeY,
    raStar,
    decStar,
    ...
    ccdCode, xCCDpix, yCCDpix = getCCDandPixelCoordinates(
        raStar,
        decStar,
        raPlatform,
        decPlatform,
    ...
    if (
        xCCDpix - subfieldSizeX / 2 < 0
        or xCCDpix + subfieldSizeX / 2 - 1 > Ncols - 1
        or yCCDpix - subfieldSizeY / 2 < firstRow
        or yCCDpix + subfieldSizeY / 2 - 1 > Nrows - 1
    ):
        return None, None, None
...
def skyToPixelCoordinates(sim, raStar, decStar, normal):
    ...
    if sim["PSF/Model"] == "MappedFromFile":
        ...
        or sim["Camera/IncludeFieldDistortion"] == True
    ):
    ...
    solarPanelOrientation = np.deg2rad(
        float(sim["Platform/Orientation/Angles/SolarPanelOrientation"])
    )
    focalPlaneAngle = np.deg2rad(
        float(sim["Camera/FocalPlaneOrientation/ConstantValue"])
    )
    ...
    ccdCode, xCCDpixel, yCCDpixel = getCCDandPixelCoordinates(
        raStar,
        decStar,
        raPlatform,
        decPlatform,
...
def pixelToSkyCoordinates(sim, ccdCode, xCCDpix, yCCDpix):
    ...
    if sim["PSF/Model"] == "MappedFromFile":
        ...
        inverseDistortionCoefficients = sim[
            "Camera/FieldDistortion/ConstantInverseCoefficients"
        ]
        pathToPsfFile = None
        mappedDistortion = False
        includeFieldDistortion = True
    ...
    xFPmm, yFPmm = pixelToFocalPlaneCoordinates(
        xCCDpix, yCCDpix, pixelSize, ccdZeroPointX, ccdZeroPointY, ccdAngle
    )
    ...
    if includeFieldDistortion:
        if mappedDistortion:
            xFPmm, yFPmm = mappedDistortedToUndistortedFocalPlaneCoordinates(
                xFPmm, yFPmm, pathToPsfFile, focalLength
            )
        else:
            xFPmm, yFPmm = distortedToUndistortedFocalPlaneCoordinates(
                xFPmm, yFPmm, inverseDistortionCoefficients, focalLength
            )

    # Get the corresponding sky coordinates

    ra, dec = focalPlaneToSkyCoordinates(
        xFPmm,
        yFPmm,
        raPlatform,
        decPlatform,
...
def perturbPlatformPointing(x, y, z, ra, dec):
    ...
    R = np.array([[0, -z, y], [z, 0, -x], [-y, x, 0]])

    # Equatorial rotation matrix

    A = np.array([[np.cos(ra) * np.sin(dec)], [np.sin(ra) * np.sin(dec)], [1]])
